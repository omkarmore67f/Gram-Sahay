...
export const LoginPage: React.FC<LoginPageProps> = ({ onGoToDashboard }) => {
  ...
  // localStorage દ્વારા mock session
  useEffect(() => {
  ...
  const onVerifyOtp = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem(
        "gramSahaySession",
        JSON.stringify({ phone, role, loggedInAt: new Date().toISOString() })
      );
    }, 800);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-amber-50 to-emerald-100 flex items-center justify-center px-4 py-6">
      ...
      <motion.main
        ...
        aria-label="ગ્રામ સહાય પંચાયત લૉગિન પેજ"
      >
        {/* ડાબી બાજુ – બ્રાન્ડ / વર્ણન */}
        <motion.section
        ...
        >
          ...
          {/* Role Toggle */}
          <div
            className="inline-flex items-center gap-1 rounded-full bg-emerald-50 p-1 mb-5"
            role="radiogroup"
            aria-label="ભૂમિકા પસંદ કરો"
          >
            <button
              type="button"
              onClick={() => handleRoleChange("user")}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-emerald-50 ${
                role === "user"
                  ? "bg-white shadow-sm text-emerald-800"
                  : "bg-transparent text-emerald-700/70"
              }`}
              aria-pressed={role === "user"}
            >
              <span className="h-5 w-5 rounded-full bg-emerald-600/10 text-[11px] flex items-center justify-center text-emerald-700">
                👤
              </span>
              ગામજનો
            </button>
            ...
            >
              <span className="h-5 w-5 rounded-full bg-amber-600/10 text-[11px] flex items-center justify-center text-amber-700">
                🏛️
              </span>
              પંચાયત / એડમિન
            </button>
            ...
            {step === "success" && (
              ...
              >
                ...
                <p className="text-xs text-emerald-900/75 max-w-sm mx-auto">
                  ...
                  તરીકે લૉગિન છો. આગળના સ્ટેપમાં તમને ડેશબોર્ડ, ફરિયાદ, નોટિસ અને
                  યોજનાઓના પેજ પર લઈ જવાશે (ડેમો માં અહીં સુધી).
                </p>
